...
// Modes: road transport (OSM roadways; formal buses navy, paratransit amber)
// and Cairo Metro (railway=subway/rail graph, per-line colors).
// Usage: node pipeline/build.mjs [--all | lines...] [--tram all|M1,M2]
// (the metro rides in the engine's "tram" slot — same graph machinery.)
// Cairo quirk: TWO TfC feeds. The road feed is all route_type 3; agencies are
// what separates formal buses (CTA, CTA_M, MM, GRN, LTRA_M) from paratransit
// (P_O_14 microbus, P_B_8 tomnaya, COOP minibus, BOX) — and hundreds of
// paratransit routes share ONE short name ("Microbus"), so lines are re-keyed
// (see the re-keying block below). Paratransit reuses the GZM "metroline"
// machinery (mline/mstop/mall/msome props + amber) for its independent toggle.
...
// m — longer jumps between shape points are GTFS data gaps. The TfC road feed
// samples shapes at ~33 m median (p90 172 m, p99 577 m, holes to 4.4 km), so the
// threshold sits above the normal spacing; inside a real gap the HMM bridges by
// routing instead of interpolating observations, which would fabricate
// straight-line detours through side streets.
const GAP_MIN = 300;
...
// Cairo paratransit (microbus/tomnaya/minibus/box) — amber, apart from the navy
...
// Two feeds, one mode each — no routeTypes filter needed (the road feed is all
// route_type 3; agency-based splitting happens in the re-keying block instead).
const MODES = [{
  mode: 'bus', label: 'road transport', gtfsDir: 'data/gtfs', osmFile: 'data/osm/cairo.json',
  graphMode: 'road', color: '#0059a9', colorDark: '#00294f',
  all: busAll, lines: busList.length ? busList : (busAll ? [] : ['1145']),
}];
const tramAll = tramLines.length === 1 && tramLines[0] === 'all';
// Cairo Metro colors as on the official diagrams: Line 1 blue, Line 2 red
// (L1 is a brighter azure here so it never reads as the navy bus color).
if (tramLines.length) MODES.push({
  mode: 'tram', label: 'metro', gtfsDir: 'data/gtfs-metro', osmFile: 'data/osm/cairo-metro.json',
  graphMode: 'tram', color: '#0088ce', colorDark: '#00466b',
  lineColors: { M1: '#0088ce', M2: '#e30613' },
  lineColorsDark: { M1: '#00466b', M2: '#7c0009' },
...
async function processMode(cfg) {
  ...
  // Line re-keying: 511 routes are named just "Microbus", so route_short_name
  // cannot be the line key as-is. Formal agencies keep their route number
  // ("CTA 1145" → "1145", "Minibus 112" → "112m" to dodge the 13 CTA/CTA_M
  // number collisions, "MM NA13" → "NA13", "Green G1" → "G1"); paratransit
  // agencies get compact per-agency codes assigned in route_id order — stable
  // for a given feed version, and short enough for the terminus badge boxes.
  // Same-key routes (e.g. two "CTA 1073" branches) merge into one line, which
  // is what a rider expects. The paratransit key set reuses the GZM
  // "metroline" machinery (independent amber category with its own toggle).
  if (cfg.mode === 'bus') {
    ...
    const formal = new Set(routes.filter((r) => !cfg.mlineSet.has(r.route_short_name)).map((r) => r.route_short_name));
    log(`paratransit lines (${cfg.mlineSet.size} routes as amber), formal lines (${formal.size})`);
  }
  let LINES = cfg.all
